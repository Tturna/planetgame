...
namespace Inventory
{
    public static class PlaceableUtility
    {
        ...
        private static (Collider2D, Transform) GetClosestNeighbor(Vector3 mousePoint, int neighborCount)
        {
            ...
            return (closestNeighbor, closestRoomTransform);
        }

        private static bool TryGetStackedBuildingPosition(RoomModuleSo roomModuleSo,
            Vector3 mousePoint,
            int neighborCount,
            int buildingBoundsMask,
            out Vector3? position,
            out Vector3? normal)
        {
            var (closestNeighbor, closestRoomTransform) = GetClosestNeighbor(mousePoint, neighborCount);
        ...
        public static bool TryGetPlaceablePosition(Vector3 mousePoint, ItemSo equippedItemSo, out Vector3? position, out Vector3? normal)
        {
            ...
            if (equippedItemSo is RoomModuleSo roomModuleSo)
            {
                ...
                if (neighborCount > 0)
                {
                    return TryGetStackedBuildingPosition(roomModuleSo, mousePoint, neighborCount, buildingBoundsMask, out position, out normal);
                }
            }
            ...
            var planetDirection = PlayerController.instance.DirectionToClosestPlanet;
            var rayHit = Physics2D.Raycast(mousePoint, planetDirection, placementAssistRange, mask);
            var alt = Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl);

            if (rayHit)
            {
                position = rayHit.point;
                normal = alt ? -planetDirection : rayHit.normal;
            }
            else
            {
                ...
                normal = alt ? -planetDirection : (mousePoint - (Vector3)position).normalized;
            }
